...
#--------------------------------------------------------------------------------------------------
fig, axarr = plt.subplots(2, 1, figsize=(8,12), sharex='col', sharey='row')
fig.subplots_adjust(hspace=0.001, wspace=0.001)
fig.subplots_adjust(bottom=0.06, top=0.98, left=0.11, right=0.98)
...
axarr[1].plot(star_data10.x_data, star_data10.y_data, linestyle='-', color='red', lw=1.0, label='Stars')
axarr[1].plot(star_data50.x_data, star_data50.y_data, linestyle='-', color='red', lw=1.0)
axarr[1].plot(star_data90.x_data, star_data90.y_data, linestyle='-', color='red', lw=1.0)
axarr[1].plot(gas_data10.x_data, gas_data10.y_data, linestyle='--', color='black', lw=1.0, label='Gas')
axarr[1].plot(gas_data50.x_data, gas_data50.y_data, linestyle='--', color='black', lw=1.0)
axarr[1].plot(gas_data90.x_data, gas_data90.y_data, linestyle='--', color='black', lw=1.0)
axarr[1].text(tmin + 0.03*tsize, 0.7, "$10\%$", fontsize=12)
axarr[1].text(tmin + 0.03*tsize, 2.0, "$50\%$", fontsize=12)
axarr[1].text(tmin + 0.03*tsize, 5.0, "$90\%$", fontsize=12)
axarr[1].text(tmin + 0.03*tsize, rmax - 0.3*rsize, "(b) $N_s = 500$, $N_g = 5000$", fontsize=12)
legend = axarr[1].legend(loc='upper right', fontsize=12)

plt.show()
fig.savefig('plummer.eps', dpi=50)
