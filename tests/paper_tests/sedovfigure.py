...
fig, axarr = plt.subplots(2, 4, sharex='col', sharey='row', figsize=(14,7))
...
#axarr[1,0].set_ylabel(r"$v_{_R}$", fontsize=14)
#axarr[1,0].set_xlabel(r"$x$", fontsize=14)
...
#axarr[1,1].set_xlabel(r"$x$", fontsize=14)
#axarr[1,1].scatter(R3[::stride], v3[::stride], color='black', marker='.', s=1.0)
...
#axarr[1,2].legend(fontsize=12)
#axarr[1,2].set_xlabel(r"$x$", fontsize=14)
#axarr[1,2].scatter(R1[::stride], v1[::stride], color='black', marker='.', s=1.0)
...
axarr[0,3].scatter(R7[::stride], rho7[::stride], color='black', marker='.', s=1.0, label='Grad-h, L=10 + S10')
axarr[0,3].plot(rhodata.x_data, rhodata.y_data, color="red")
axarr[0,3].text(xmin + 0.65*xsize, rhomax - 0.08*rhosize, r'$L = 10 + {\rm S10}$', color='black', size=14)
#axarr[0,3].legend(fontsize=12)
axarr[1,3].set_xlabel(r"$x$", fontsize=14)
axarr[1,3].set_xlim([xmin, xmax])
axarr[1,3].set_ylim([rhomin, rhomax])
axarr[1,3].scatter(R3[::stride], rho3[::stride], color='black', marker='.', s=1.0, label='MFV, L=10 + S10')
axarr[1,3].plot(rhodata.x_data, rhodata.y_data, color="red")
#axarr[1,3].legend(fontsize=12)
#axarr[1,3].set_xlabel(r"$x$", fontsize=14)
#axarr[1,3].scatter(R4[::stride], v4[::stride], color='black', marker='.', s=1.0)
...
#axarr[0,4].scatter(R2[::stride], rho2[::stride], color='black', marker='.', s=1.0, label='MFV, L=10 + SM')
#axarr[0,4].plot(rhodata.x_data, rhodata.y_data, color="red")
#axarr[0,4].legend(fontsize=14)
#axarr[1,4].set_xlabel(r"$x$", fontsize=14)
#axarr[1,4].scatter(R2[::stride], v2[::stride], color='black', marker='.', s=1.0)
#axarr[1,4].plot(vxdata.x_data, vxdata.y_data, color="red")


# Gradh-SPH with 10 timestep levels + SM limiter
#axarr[0,5].set_xlim([xmin, xmax])
#axarr[0,5].set_ylim([rhomin, rhomax])
#axarr[0,5].scatter(R5[::stride], rho5[::stride], color='black', marker='.', s=1.0, label='Gradh, L=10 + SM')
#axarr[0,5].plot(rhodata.x_data, rhodata.y_data, color="red")
#axarr[0,5].legend(fontsize=14)
#axarr[1,5].set_xlabel(r"$x$", fontsize=14)
#axarr[1,5].scatter(R5[::stride], v5[::stride], color='black', marker='.', s=1.0)
#axarr[1,5].plot(vxdata.x_data, vxdata.y_data, color="red")
