...
Ewald<ndim>::Ewald(DomainBox<ndim> &simbox, int _gr_bhewaldseriesn, int _in, int _nEwaldGrid,
                   DOUBLE _ewald_mult, DOUBLE _ixmin, DOUBLE _ixmax):
...
// set ewald_periodicity for given type of boundary conditions
  ...
  if (ewald_periodicity == 0) {
    printf("Ewald method does not work with fully isolated boundary conditions");
    exit(0);
  }
...
//  ewald_coord = new DOUBLE[3*one_component];
 ...
 // add parallelization !!!
        ...
        x = i*Lewald[0]/Ncells[0];
        y = j*Lewald[1]/Ncells[1];
        z = k*Lewald[2]/Ncells[2];

//  printf("EF %d %d %d %d %16.10lf %16.10lf %16.10lf\n",i,j,k,ewald_index,x,y,z);

        ewald_field[ewald_index] = 0.0;
        ewald_fields[ewald_index] = 0.0;
        ewald_fieldl[ewald_index] = 0.0;
//        ewald_coord[ewald_index] = x;
//        ewald_coord[ewald_index+one_component] = y;
//        ewald_coord[ewald_index+2*one_component] = z;
...
// the second part of the condition means subtraction of 1/r term
...
// fields ewald_fields and ewald_fields serve only for the sake of debugging
                ewald_field[ewald_index] = ewald_field[ewald_index] + erfc(ewald_alpha*rni)/rni;
                ewald_fields[ewald_index] = ewald_fields[ewald_index] + erfc(ewald_alpha*rni)/rni;
//if ((i == 1) && (j == 0) && (k == 0)) {
//            printf("test %d %d %d  %d %d %d %10.6e %10.6e\n",ni,nj,nk,i,j,k,i*simbox.boxhalf[0],i*simbox.boxhalf[0]/(nEwaldGrid-1));
//}
                for (jj=1; jj<4; jj++) {
                  ewald_field[ewald_index+jj*one_component] = ewald_field[ewald_index+jj*one_component] + AccShort(ewald_alpha,rrel[jj],rni);
                  ewald_fields[ewald_index+jj*one_component] = ewald_fields[ewald_index+jj*one_component] + AccShort(ewald_alpha,rrel[jj],rni);
                }
              }
            }
          }
            ...
            for (hk=-es_nfz; hk<=es_nfz; hk++) {
              if ((cf1*hi*hi+cf2*hj*hj+cf3*hk*hk) <= es_radius2) {
                ...
                if (ewald_periodicity==1) {
                  ...
                  ewald_fieldl[ewald_index] = ewald_fieldl[ewald_index] + PotLong1p2i(hi, ewald_dzeta, linv, x, y, z);
                  ...
                  ewald_fieldl[ewald_index+one_component] = ewald_fieldl[ewald_index+one_component] + 
                      linv_acc*(AccLong1p2iPer(hi, ewald_dzeta, linv, x, y, z));
                  ...
                  ewald_fieldl[ewald_index + 2*one_component] = ewald_fieldl[ewald_index + 2*one_component] + 
                      linv_acc*(AccLong1p2iIso(hi, ewald_dzeta, linv, x, y, z));
                  ...
                  ewald_fieldl[ewald_index + 3*one_component] = ewald_fieldl[ewald_index + 3*one_component] +  
                      linv_acc*(AccLong1p2iIso(hi, ewald_dzeta, linv, x, z, y));
//        printf("%d %d %d %e %e \n",hi,hj,hk,linv_acc*(AccLong1p2iIso(hi, ewald_dzeta, linv, x, y, z)),linv_acc);
                      ...
                      ewald_fieldl[ewald_index] = ewald_fieldl[ewald_index] + 
                          PotLong2p1i(ewald_dzeta, linv, ratio_pinv, x, y, z, hi, hj, ewc1, ewc2);
                      ewald_field[ewald_index+one_component] = ewald_field[ewald_index+one_component] + 
                    ...
                    if ((pow(hi,2)+pow(hj,2)+pow(hk,2)) > 0) {
                      ...
                      ewald_fieldl[ewald_index + 2*one_component] = ewald_fieldl[ewald_index + 2*one_component] + 
                          ratio_pinv1*(AccLong3pPer(hj, linv_acc, ewald_dzeta, ewc3, ewc4));
//     on the two following lines was a bug (missing ratio_pinv2) - it may preserve in one of the previous versions !!!
                      ewald_field[ewald_index + 3*one_component] = ewald_field[ewald_index + 3*one_component] + 
...
// determine the constant potC1p2i in the 1p2i case and correct for -1/r term
  if (ewald_periodicity==1) {
    potC1p2i = 2.0*linv*log(2*lx_per) + ewald_field[Ngrid[0]*(Ngrid[1]-1)];
  } else if (ewald_periodicity==2) {
    potC1p2i = 2.0*linv*log(2*ly_per) + ewald_field[Ngrid[0]-1];
  } else if (ewald_periodicity==4) {
    potC1p2i = 2.0*linv*log(2*lz_per) + ewald_field[Ngrid[0]-1];
  }

  potC1p2i = -(potC1p2i + 0.5/lx_per);

        printf("potC1p2i %16.10lf %16.10lf %16.10lf %16.10lf\n",potC1p2i,2.0*linv*log(2*lx_per),
           ewald_field[Ngrid[0]*(Ngrid[1]-1)],ewald_field[Ngrid[0]*(Ngrid[1]-1)]+0.5/lx_per);
}
...
//    if (fmax(fmax(il[0],il[1]),il[2]) >= 4*(nEwaldGrid-1)) {
    if (max3(ip[0],ip[1],ip[2]) > 4*(nEwaldGrid-1)) {
...
// the term 1/r or 1/r**2 is subtracted to compensate for the major contribution
      ...
      switch (ewald_periodicity) {
        ...
        case 3:
          ...
          acorr[2] = m*(accPlane - dr[2]*pow(drAbsInv,3));
          break;

      }
//  printf("%8d %8d %8d %8d %16.10lf %16.10lf %16.10lf %16.10lf  %16.10lf \n",il[0],il[1],il[2],dr[0],dr[1],dr[2],
 //    gpotcorr, acorr[2]);
      return;
    }
  }
...
//  printf("%8d %8d %8d %8d %8d %8d %8d %16.10lf %16.10lf %16.10lf %16.10lf\n",il[0],il[1],il[2],indexBase2,indexBase2 + Ngrid[0],
//  indexBase3, indexBase3 + Ngrid[0], grEwald[0],grEwald[3],ewald_field[indexBase0],ewald_field[indexBase1+3*one_component]);

  gpotcorr = m*grEwald[0];
  for (k=0; k<ndim; k++) {
//    acorr[k] = m*copysign(grEwald[k+1],dr[k]);
    acorr[k] = m*grEwald[k+1]*sgn(dr[k]);
  }
//  printf("Int %16.10lf %16.10lf %16.10lf \n",);
...
//  printf("int %d %10.5lf\n",l,SimpsonInt(IntFPot,l,ewald_dzeta,ewald_eta,ixmin,ixmax,in));
  return(a);
}
