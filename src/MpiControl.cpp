  ...
  bool& initial_h_provided)            ///< Receives from root whether or not initial h was provided
{
  ...
  if (rank == 0) {
    ...
    if (simbox.boundary_lhs[0] == "open") mpibox.boxmin[0] = -big_number;
    else mpibox.boxmin[0] = simbox.boxmin[0];
    if (simbox.boundary_rhs[0] == "open") mpibox.boxmax[0] = big_number;
    else mpibox.boxmax[0] = simbox.boxmax[0];
    if (ndim > 1) {
      if (simbox.boundary_lhs[1] == "open") mpibox.boxmin[1] = -big_number;
      else mpibox.boxmin[1] = simbox.boxmin[1];
      if (simbox.boundary_rhs[1] == "open") mpibox.boxmax[1] = big_number;
      else mpibox.boxmax[1] = simbox.boxmax[1];
    }
    if (ndim == 3) {
      if (simbox.boundary_lhs[2] == "open") mpibox.boxmin[2] = -big_number;
      else mpibox.boxmin[2] = simbox.boxmin[2];
      if (simbox.boundary_rhs[2] == "open") mpibox.boxmax[2] = big_number;
