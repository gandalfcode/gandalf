...
public:
  ...
  GhostNeighbourFinder(const DomainBox<ndim>& simbox) : _domain(simbox),
  ...
  //===============================================================================================
  inline void SetTargetCell(const TreeCellBase<ndim>& cell)
  {
    _need_mirrors = false;
    for (int k=0; k < ndim; k++) {
  ...
  /// \return A boolean saying whether the boxes overlap
  //===============================================================================================
  inline type_flag NearestPeriodicVector(FLOAT dr[ndim]) const
  {
    type_flag bound;
    if (_any_periodic) {
      for (int k=0; k<ndim; k++) {
  ...
  /// \return A boolean saying whether the boxes overlap
  //===============================================================================================
  inline type_flag PeriodicDistanceCorrection(const FLOAT dr[ndim], FLOAT dr_corr[ndim]) const
  {
    type_flag bound;
    if (_any_periodic) {
      for (int k=0; k<ndim; k++) {
  ...
  inline int ConstructGhostsScatterGather(const ParticleType<ndim>& p, ParticleType<ndim>* ngbs) const
  {
    ngbs[0] = p;
    if (_any_special == false) return 1;
  ...
  //===============================================================================================
  template<template <int> class ParticleType>
  inline int ConstructGhostsGather(const ParticleType<ndim>& p, ParticleType<ndim>* ngbs) const
  {
    ngbs[0] = p;
    if (_any_special == false) return 1;
    ...
    // Now recursively reflect the cells
    int Nghost = 1;
    if (_need_mirrors) Nghost = _MakeReflectedGhostsGather(ngbs);
  ...
  inline void CorrectGhostParticlePosition(const TreeCell<ndim>& cell, const FLOAT *r,
                                           const int *sign, ParticleType<ndim> &p) const
  {
    if (_any_special) {
      for (int k=0; k < ndim; k++) {
...
private:
  ...
  //===============================================================================================
  template<template <int> class ParticleType>
  inline void _MakePeriodicGhost(ParticleType<ndim>& p) const
  {
    FLOAT dr[ndim];

    for (int k=0; k <ndim; k++) dr[k] = p.r[k] - _centre[k];

    type_flag bound_flag =  NearestPeriodicVector(dr);
    for (int k=0; k <ndim; k++) p.r[k] = _centre[k] + dr[k];
  ...
  //===============================================================================================
  template<template <int> class ParticleType>
  inline int _MakeReflectedGhostsGather(ParticleType<ndim>* ngbs) const
  {
    int nc = 1;
  ...
  //===============================================================================================
  template<template <int> class ParticleType>
  inline int _MakeReflectedScatterGatherGhosts(ParticleType<ndim>* ngbs) const
  {
    int nc = 1;
...
//=================================================================================================
template<int ndim>
struct ParticleCellProxy : public TreeCellBase<ndim>
{
...
//=================================================================================================
template<int ndim>
struct DomainCellProxy : public TreeCellBase<ndim>
{
  using TreeCellBase<ndim>::hboxmin;
  using TreeCellBase<ndim>::hboxmax;
  using TreeCellBase<ndim>::rcell;
