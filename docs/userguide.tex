...
\item FFTW : Include FFTW (Fast Fourier transform) library for initial conditions (0 or 1)

\item GSL : Include GSL (GNU Scientific library) for Ewald forces (0 or 1)
...
godunov\_sph      & = Godunov SPH simulation (Inutsuka 2002) \\
...
rad\_ws     & = EOS relating to Stamatellos et al. (2007) cooling method (to be used with rad\_ws option in energy equation)
\end{tabular}
...
octtree    & = Barnes-Hut Octal tree (partially working)
\end{tabular}
...
monoionisation  &= Monte-Carlo radiation transport using a kd-tree
\end{tabular}
...
\item \var{pruning\_level} : Level to prune exported trees down to
