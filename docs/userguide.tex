...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\subsubsection{Example 3 - Creating and running a new simulation using the internal initial conditions routines without a parameters file}
...
Finally, we can import the arrays into the main C++ and run the simulation.  Before doing this, we must first initialise various things in the code, including importantly allocating memory for the particles, by calling the function \\
\newline
...
\newline
where \var{numpyarray} is the local NUMPY array and \var{varname} is the name of the variable in C++.  For example, to import the x-positions, we call the command \var{sim.ImportArray(x,`x')}.  Once all arrays have been called, we can finally call the setup function \var{sim.SetupSimulation()} and plot and run the simulation.
\newpage
