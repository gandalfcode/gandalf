...
\begin{itemize}
\item CODENAME has been written with a fully object-oriented philosophy in C++.  This is to improve the maintainability of the code and also to allow the code to be easily extended in the future with as little disruption to the original code-base as possible.
\item CODENAME contains a variety of SPH algorithms such as conservative 'grad-h' SPH, Saitoh \& Makino (2013) SPH and Godunov SPH.  The N-body component contains a variety of common integration schemes such as 4th/6th-order Hermite with time-symmetric integration.
\item CODENAME can be run in three different modes; as a standard executable run from the command line (like SEREN), run from a python script or run inside an interactive python environment.  Both the script and interactive python modes include a graphical output that can be used to visualise previously run simulations, or to interactively visualise simulations as they are run on-the-fly.
\item CODENAME can generate initial conditions at run-time, as opposed to SEREN where the initial conditions had to be prepared a priori.  The initial conditions may either be hard-coded into the C++ code, or set-up via a python script using numpy arrays.
\item CODENAME uses additional parameters to switch on various physics options, as well as requiring them to be activated in the Makefile.  In comparison, SEREN controlled physics options exclusively in the Makefile.  Therefore, CODENAME need not be re-compiled everytime a different setting is required.
\end{itemize}
...
To compile CODENAME only as a C++ executable to be run from the command line without visualisation via the python program, first set the chosen C++ compiler in the Makefile and type \\
\newline
...
\newline
\noindent \var{export PYTHONPATH=XXX/YYY/codename:\$PYTHONPATH} \\

\noindent to your bashrc (or bash\_profile) script where {\var XXX/YYY/codename} is the absolute path of the CODENAME directory.   If you are using csh, tcsh or related shells, then add
\newline
\noindent \var{setenv PYTHONPATH ``XXX/YYY/codename:\$PYTHONPATH''} \\
...
\begin{tabular}{ll}
lfkdk        & = Leapfrog kick-drift-kick \\
hermite4     & = 4th-order Hermite scheme \\
hermite4ts   & = Time-symmeric 4th-order Hermite scheme
\end{tabular}

\item \var{ic} : Simulation initial conditions \vspace{0.1cm} \\
\begin{tabular}{ll}
binary            & = Simple binary star test \\
boss\_bodenheimer & = Boss-Bodenheimer (1979) test \\
...
\item \var{endtime} : Termination time of the simulation (given in {\var tunit}s)
...
%\item \var{grav\_kernel} : Use kernel-softened gravity
...
\var{sim.SetParam('Npart',200)} & 200 SPH particles \\
