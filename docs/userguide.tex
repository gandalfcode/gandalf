...
\newpage 
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\subsection{GANDALF Python library tutorial}
...
We provide here a short tutorial demonstrating from the most basic usage to the more advanced functionality of the Python library. \\
\newline
...
\var{from gandalf.analysis.facade import *} \\
\var{sim = newsim(`params.dat')} \\
\var{sim.SetParam(`tend',2.0)} \\
\var{setupsim()} \\
\var{run()} \\
\end{tabular} \\
\newline

The first two lines of this example are the as the first example, where we import the GANDALF Python frontend and then create a new simulation from the parameters file \var{`params.dat'}.  Once loaded in, we can now modify parameters with the command \\
\newline
\noindent \var{sim.SetParam(parameter\_name,new\_value)} \\
\newline
\noindent In this case, we set the parameter \var{tend} (the total simulation run-time) to the value \var{2.0} with \var{sim.SetParam(`tend',2.0)}.  Once all parameters have been modified, then we can set-up the simulation fully with \var{setupsim()} and run it with \var{run()}.  Note that if we attempt to modify any parameters AFTER calling \var{setupsim()}, then the code will terminate with an exception.
...
\begin{tabular}{p{14.0cm}}
\var{from gandalf.analysis.facade import *} \\
\var{sim = newsim(ndim=2)} \\
\var{sim.SetParam(`sim',`sph')} \\
...
In this example, we call the \var{newsim} function passing the dimensionality of the simulation instead of a parameter file.  Therefore the line \var{sim = newsim(ndim=2)} sets up a 2-dimensional simulation, but is otherwise unspecified because no other parameters have been set.  Next, we can set all the parameters that would have otherwise been set in the parameters file.  In this example, we set up an SPH simulation (\var{sim.SetParam(`sim',`sph')}) using the grad-h SPH method (\var{sim.SetParam(`sph',`gradh')}) with initial conditions to perform the Sedov blast-wave test (\var{sim.SetParam(`ic',`sedov')}); (See Section \ref{S:PARAMETERS} for a full description of all parameters).  Once all parameters have been set, we can fully set-up and run the simulation with \var{setupsim()} and \var{run()}.
...
\var{plot(x,y)} \\
\newline
\noindent where \var{x} and \var{y} are the x- and y-axis plotting variables (N.B. these can be any variables, not necessarily x and y. e.g. \var{plot(`y',`z')}).  If we now run the simulation with \var{run()}, the simulation will compute as normal while updating the plot window every \var{dt\_python} seconds (in this casem every 2 seconds).  After the simulation has finished, we can save the final plot to file with the command \\ 
\newline
\noindent \var{savefig(filename)} \\
\newline
\noindent where \var{filename} is the intended filename {\it including} the file extension (N.B. matplotlib automatically determines the correct file format from the extension).
...
\var{Nsph = 200} \\
\var{x = np.linspace(-0.995,0.995,num=Nsph)} \\
\var{m = np.ones(Nsph)*2.0/Nsph} \\
\var{u = np.ones(Nsph)*1.5} \\
\var{vx = np.zeros(Nsph)}  \\
\var{for i in range(Nsph):} \\
\hspace{0.5cm}\var{if x[i] < 0.0:} \\
...
\newline
\noindent \var{import numpy as np} \\
\newline
\noindent
We then create an undefined 1D simulation object with \var{sim = newsim(ndim=1)} and set various parameters determing the simulation type, integration scheme, equation of state, etc..  We must then inform the code that we intend to generate initial conditions in the python script (and not say from an external file or one of the internal C++ routines) by the command \var{sim.SetParam(`ic',`python')}.  We must also inform the code of the number of SPH particles to be used with the command \var{sim.SetParam(`Nsph',200)}, in order to allocate enough memory for the simulation.  

At this point, we can now create the initial conditions in Python.  This can be done by creating NUMPY arrays for each variable, e.g. \var{x}, \var{m}, etc..  Note that even for vector quantities (e.g. position, velocity), we must allocate separate 1D NUMPY arrays for each component, for example the commands \\
\newline
\noindent \var{a1 = np.zeros(N)}, \\
\noindent \var{a2 = np.ones(N)}, \\
\noindent \var{a3 = np.linspace(0.0,1.0,num=N)} \\
\newline
...
\newline
\noindent \var{sim.PreSetupforPython()}\,. \\
\newline
\noindent
Next, we can import the array into the C++ arrays by the command \\
\newline
\noindent \var{sim.ImportArray(numpyarray,varname)} \\
\newline
...
\var{from gandalf.analysis.facade import *} \\
\var{loadsim(`TEST1')} \\
\var{render(`x',`y',`rho',res=256)} \\
\var{savefig(`rendercolumn.png')} \\
\var{render(`x',`y',`rho',res=256,zslice=0.0)} \\
\var{savefig(`renderslice.png')} \\
\var{block()} \\
\end{tabular} \\
\newline

Once the GANDALF library and simulation snapshot files have been loaded in the script (or alternatively during a live simulation), rendered images from the snapshots can be generated with command \\
\newline
\noindent \var{render(x,y,renderquantity)} \\
\newline
\noindent where \var{x} and \var{y} are the x- and y-axis quantities (similar to the \var{plot} command) and \var{renderquantity} is the quantity to be rendered.  There are various optional arguments, such as \var{res} which controls the resolution.  By default, \var{render} generates a column rendered image (for 3D simulations).  Therefore, the command \var{render(`x',`y',`rho',res=256,zslice=0.0)} generates a column density plot in the x-y plane using $256$ pixels in each direction.  To generate a rendering of a slice instead of a column, the optional argument \var{zslice} (i.e. the coordinate of the slice in the z-direction) must be included.  Therefore, the command \var{render(`x',`y',`rho',res=256,zslice=0.0)} generates a rendering of density along a slice at \var{zslice = 0.0}.



%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\subsubsection{Example  11 - Plotting in alternative coordinate systems}
GANDALF contains three coordinates systems, Cartesian, Spherical polar and Cylindricalc oordinates.  This example demonstrates how to plot in the different systems.
...
\var{loadsim(`TEST1')} \\
\var{plot(`x',`y')} \\
\var{savefig(`cartesian.png')} \\
\var{plot(`r',`rho')} \\
\var{savefig(`sphericalpolar.png')} \\
\var{plot(`R',`rho')} \\
\var{savefig(`sphericalpolar.png')} \\
\end{tabular} \\
\newline
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\subsubsection{Example  16 - Creating and plotting on multiple windows}
To be done
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\subsubsection{Example  18 - Creating an animation from simulation snapshot files}
To be done
...
\item Ability to create animations/movies from simulation snapshots
